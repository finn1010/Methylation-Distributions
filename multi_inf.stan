...
parameters{
    ...
    //array[J] real<lower=1,upper=(90)> t;          //CNA time - add upper bound
...
transformed parameters {
    ...
    //array[J] real<lower=0> gamma = gamma_raw * mu;
    array[J] real gamma;
   // array[J] real t;
    for (j in 1:J) {
      //  t[j] = t_raw[j] * age[j]; 
    ...
    array[J] real t;  // Event time for each patient

    for (j in 1:J) {
        // Transform theta_t[j] to ensure t[j] is between 0 and age[j]
        t[j] = age[j] * exp(theta_t[j]) / (1 + exp(theta_t[j]));
    }
...
model {
    //theta/CNA time are flat priors
    kappa ~ lognormal(3.6, 0.5);       



   // gamma ~ normal(0,0.05);
   // mu_mean ~ normal(0, 0.1);  // Prior on mu_mean
   // mu_sd ~ normal(0, 0.05);   // Prior on mu_sd
   // mu_raw ~ normal(0, 1);     // Standardized prior on mu_raw
